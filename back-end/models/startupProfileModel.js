...
const StartupProfile = {
    ...
    async create(profile) {
        const [result] = await db.query(
            `INSERT INTO startup_profile_details (user_id, username, company_name, logo, logo_mime, description, tam, sam, som) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                profile.user_id,
                profile.username,
    ...
    async updateByUserId(userId, fields) {
        const setParts = [];
        const values = [];
        for (const key of ['company_name', 'logo', 'logo_mime', 'description', 'tam', 'sam', 'som']) {
