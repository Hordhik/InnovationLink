...
const Inbox = () => {
  ...
  const [activeUser, setActiveUser] = useState(initialChat ? { id: 0, name: initialChat.companyName || initialChat.username || 'New chat' } : allUsers[0]);
  ...
  // If we arrived with an initialChat target, ensure it is selected
  useEffect(() => {
    if (initialChat) {
      setActiveUser({ id: 0, name: initialChat.companyName || initialChat.username || 'New chat' });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [initialChat?.username, initialChat?.companyName]);
